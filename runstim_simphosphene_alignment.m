function runstim_simphosphene_alignment(Hnd)
...
%         targetLocations='LRTB';
    ...
    spacing=sampleSize;%present 2 simulated phosphenes 
    if LRorTB==1
        if targetLocation==1%L
            finalPixelCoordsAll=[sampleX-spacing/2 sampleY-spacing/2;sampleX+spacing/2 sampleY+spacing/2];%calculate coordinates of selected pixel locations
        elseif targetLocation==2%R
            finalPixelCoordsAll=[sampleX+spacing/2 sampleY-spacing/2;sampleX-spacing/2 sampleY+spacing/2];
        end
    elseif LRorTB==2
        if targetLocation==1
            finalPixelCoordsAll=[sampleX sampleY-spacing/2;sampleX sampleY+spacing/2];
        elseif targetLocation==2
            finalPixelCoordsAll=[sampleX-spacing/2 sampleY;sampleX+spacing/2 sampleY];
        end
    end
    numSimPhosphenes=length(finalPixelCoordsAll);
...
%                 end
    ...
    if Hit == 0 %subject kept fixation, display stimulus
        ...
        if Hit == 0 %subject kept fixation, subject may make an eye movement
            ...
            if LRorTB==1
                distBetweenDots=10;%the distance between the two dots comprising the target stimuli, in x- and y-coordinates (not in terms of the shortest path between them)
                %left target:
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(1)-distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(1)-distBetweenDots/2 screenWidth/2+targetArrayX(1)-distBetweenDots/2 screenHeight/2+targetArrayY(1)-distBetweenDots/2]);
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(1)+distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(1)+distBetweenDots/2 screenWidth/2+targetArrayX(1)+distBetweenDots/2 screenHeight/2+targetArrayY(1)+distBetweenDots/2]);
                %right target:
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(2)+distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(2)-distBetweenDots/2 screenWidth/2+targetArrayX(2)+distBetweenDots/2 screenHeight/2+targetArrayY(2)-distBetweenDots/2]);
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(2)-distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(2)+distBetweenDots/2 screenWidth/2+targetArrayX(2)-distBetweenDots/2 screenHeight/2+targetArrayY(2)+distBetweenDots/2]);
            elseif LRorTB==2
                distBetweenDots=14;
                %top target:
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(1) screenHeight/2-targetSize+targetArrayY(1)-distBetweenDots/2 screenWidth/2+targetArrayX(1) screenHeight/2+targetArrayY(1)-distBetweenDots/2]);
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(1) screenHeight/2-targetSize+targetArrayY(1)+distBetweenDots/2 screenWidth/2+targetArrayX(1) screenHeight/2+targetArrayY(1)+distBetweenDots/2]);
                %bottom target:
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(2)-distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(2) screenWidth/2+targetArrayX(2)-distBetweenDots/2 screenHeight/2+targetArrayY(2)]);
                Screen('FillOval',w,black,[screenWidth/2-targetSize+targetArrayX(2)+distBetweenDots/2 screenHeight/2-targetSize+targetArrayY(2) screenWidth/2+targetArrayX(2)+distBetweenDots/2 screenHeight/2+targetArrayY(2)]);
            end
